...
async def get_task_detail(id:int, db: Session = Depends(database.get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
    ...
    task = schemas.TaskDetail(title = tmp_task.title, user_name = tmp_user.name, task_date=tmp_task.register_date, skill_set = skills, concern_desc = tmp_task.concern_desc, task_detail = tmp_task.task_detail, ticket_link = tmp_task.ticket_link, slack_id = tmp_user.slack_id)
    return task
